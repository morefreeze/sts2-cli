...
- **Root cause still open**: needs managed stacks from the saved dumps (`dotnet-dump analyze` + SOS `clrstack -all`; `dotnet-dump` is not installed here), or instrumentation run against the `run_32` repro.
