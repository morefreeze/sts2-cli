...
- **ROOT CAUSE (2026-09-24, confirmed with runtime evidence)**: a temporary, user-approved diagnostic in `PillageOnPlay` (reverted, never committed) printed the loop state at iterations 2 000 and 4 000 of the hung `run_32` search: every draw is a Strike-tagged card; it does not stay in hand; `HookMirrors.ShouldPlay` returns false for it with preventer `MegaCrit.Sts2.Core.Models.Powers.RingingPower`; the boss sits at 115 HP in both snapshots (no damage in 2 000 iterations); the hand holds exactly 9 non-Strike cards (one under the 10 limit); the draw + discard piles hold only 5 Strike-tagged cards. So: Pillage draws a Strike -> Hellraiser auto-plays it -> Ringing ("you can only play 1 card this turn", applied by the boss on an earlier simulated enemy turn) vetoes the auto-play because a card was already played this turn -> `AutoPlay` moves the card to its result pile **without playing it** -> the hand never grows, the boss never takes damage, and the next draw (after a reshuffle) is another Strike. Forever.
- **It is a real-game loop, faithfully mirrored -- not a porting bug.** Every link matches the decompiled game: `Pillage.OnPlay` has the identical `do { Draw } while (Attack && hand < MaxCardsInHand)` with no exit; `CardCmd.AutoPlay` sends a card vetoed by `Hook.ShouldPlay` to `MoveToResultPileWithoutPlaying` (solver: `CombatPredictionSimulator.AutoPlay.cs:36-42`); `RingingPower.ShouldPlay` vetoes only cards carrying the Ringing affliction once a card has been played this turn (solver: `ShouldPlayMirrors.cs:74-85`, which checks the affliction too). Ringing is not active at the start of the real turn -- the hang is in a FUTURE turn the search looked ahead into, which is why the live game never hung. `PillageOnPlay` is the only `while (true)` draw loop among the solver's mirrors.
- **Fix options** (the loop lives in vendored code, so this is a policy call -- see VENDORED.md): (a) a documented local patch to `PillageOnPlay` that stops when a draw makes no progress (the drawn card left the hand and a full cycle of the draw + discard piles grew nothing) -- it only diverges from the real game in states where the real game itself would hang; (b) check whether upstream (Torch1230/CombatSolver) already fixed it and re-vendor per VENDORED.md; (c) keep relying on the harness watchdog, which already caps the cost at one game.
