...
- **Fix needed (two layers)**: (1) harness: a per-call wall-clock watchdog on `plan_combat_turn` well above the 120 s budget (e.g. 300 s) that kills/restarts the engine and records a distinct status, so a hang costs one game, not a day; (2) root cause in the solver from the dump. Reproduction should now be deterministic: replay Ironclad `run_17` with `STS2_SOLVER_CHARS=Ironclad STS2_SOLVER_THREADS=3` (map RNG is seeded per run since 233ba34).
