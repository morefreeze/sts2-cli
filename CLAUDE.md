...
- `plan_combat_turn` action (at a `combat_play` decision) returns a multi-turn plan from the ported Combat Solver engine (`src/Sts2Headless/CombatSolverEngine/`). Unlike every other action in this file, its card actions are addressed by `card_id`/`card_occurrence` (stable identity), **not** `card_index` (hand position) — the plan spans future turns whose hand contents aren't simulated at the JSON-protocol level, so a hand-position index would be meaningless past turn 0. Likewise `target_index`/`target_combat_id` for turn ≥2 actions have no counterpart in the ordinary `combat_play` decision's enemy list (which only exposes positional `index`) — **only execute the actions up through the plan's first `end_turn`, then re-call `plan_combat_turn` fresh for the next turn** rather than trying to execute a whole multi-turn plan blindly. This mirrors `agent/turn_planner.py`'s own "re-plan every step" design.
