...
- `plan_combat_turn` costs ~3.4 s median / ~9 s mean per call (up to its 120 s soft budget, `SolverSearchProfile.Default` — the only profile) versus ~0.01 s for the heuristic, so a solver-driven game takes minutes. It can also **hang indefinitely** (`agent/bug.md` BUG-040): the budget is only checked between search-node expansions and the harness has no per-call timeout, so watch long batches for a stalled game log, not just for completion.
