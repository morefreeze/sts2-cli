---
name: adk-commit
description: "Use when the user explicitly wants to commit existing changes with the TTADK commit workflow, generate a conventional commit message, or push if appropriate; do not use to modify code or commit implicitly."
---

# ADK Commit

This skill is used to commit existing changes. It is suitable for generating commit messages under TTADK conventions and pushing when appropriate.

## Manual Trigger Constraints

- Use this skill only when the user explicitly wants to commit current changes, generate a commit message under TTADK rules, or perform a push.
- This skill does not modify code. It only commits and pushes existing changes.
